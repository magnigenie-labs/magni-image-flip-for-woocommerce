=== Magni Image Flip For WooCommerce  ===
Contributors: sagarseth9,magnigenie,kshirod-patel
Tags: woocommerce,woocommerce addon,image flip, woocommerce image flip,image flip plugin, woocommerce gallery, image rotate
Donate link: https://www.paypal.com/cgi-bin/webscr?cmd=_xclick&business=sales@magnigenie.com&item_name=Magni%20Image%20Flip%20For%20WooCommerce&return=http://magnigenie.com
Requires at least: 4.0
Requires PHP: 5.4
Tested up to: 6.1.1
Stable tag: 1.1
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html 

Magni Image Flip adds a flip effect on your woocommerce product thumbnail images. It takes the first image from the gallery and uses it as a second image for the product.

== Description ==

Magni Image Flip adds a flip effect on your woocommerce product thumbnail images. It takes the first image from the gallery and uses it as a second image for the product.

The plugin allows you to have different types of animations for image hover effect. It will allow you to customize the plugin to as per your needs.

Just make sure add a second image to your WooCommerce Gallery and it will automatically display the image on your site.

= Check plugin in action =
[youtube https://www.youtube.com/watch?v=EMEVPs2npxI]

== Installation ==
1. Upload "magni-image-flip.zip" file to your "/wp-content/plugins/" directory.
2. Once uploaded go to your WordPress admin and navigate to plugins.
3. Once you are on plugins screen then you can see Magni Image Flip For WooCommerce listed there.
4. Just activate and start playing with the plugin configurations.

== Frequently Asked Questions ==
= Why should I use this plugin? =
This plugin would help you to engage more customers by creating nice looking image flip effect. 

== Screenshots ==
1. Admin settings.
2. Frontend View.

== Changelog ==

= 1.1 =
* Added: bottom left and bottom right Pager position.
* Added: Correct .pot file added.

= 1.0 =
* Initial release.

== Upgrade Notice ==
= 1.1 =
This is the latest version of our plugin. Please take a backup before the update.